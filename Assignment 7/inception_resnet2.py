...
class NaiveInceptionSimple(nn.Module):
    def __init__(self, C_in):
        super(NaiveInceptionSimple, self).__init__()

        self.tower_one   = nn.Conv2d(C_in, 6, (1, 1))
        self.tower_two   = nn.Conv2d(C_in, 6, (3, 3), padding=1)
        self.tower_three = nn.Conv2d(C_in, 6, (5, 5), padding=2)

    def forward(self, x):
        tower_one_stream   = self.tower_one(x).clamp(min=0)
        tower_two_stream   = self.tower_two(x).clamp(min=0)
        tower_three_stream = self.tower_three(x).clamp(min=0)
...
class DimensionReductionInception(nn.Module): # TODO: How is the dimensions reduced?
    def __init__(self, C_in):
        super(DimensionReductionInception, self).__init__()

        self.tower_one_1 = nn.MaxPool2d((3, 3), stride=(1, 1), padding=1)
        self.tower_one_2 = nn.Conv2d(C_in, 6, (1, 1))

        self.tower_two_1 = nn.Conv2d(C_in, 6, (1, 1))
        self.tower_two_2 = nn.Conv2d(6, 6, (3, 3), padding=1)

        self.tower_three_1 = nn.Conv2d(C_in, 6, (1, 1))
        self.tower_three_2 = nn.Conv2d(6, 6, (5, 5), padding=2)
...
class NaiveResnet(nn.Module):
    ...
    def forward(self, x):
        ...
        #output = torch.cat([x, stream], dim=1) # TODO: Morten - skal det være concatenation her? Eller skal de plusses sammen?
        #output = torch.add([x, stream], dim=1)
...
class Net(nn.Module):
    def __init__(self):
        ...
        #self.inception = DimensionReductionInception(3)#NaiveFireblock(3)#NaiveResnet(3)
        #self.naive_inception = NaiveInceptionSimple(3) # Build the model using the above modules
    ...
    def forward(self, x):
        #stream = self.res(x)#inception(x)
        #import pdb;pdb.set_trace()
        stream = self.conv(x)
        #stream = self.naive_inception(x)
